...
class Config:
    ...
    SQLALCHEMY_DATABASE_URI = DATABASE_URL or 'sqlite:///flashpair.db'
...
def check_new_image():
    ...
    new_image = Image.query.filter_by(recipient_id=current_user_id).order_by(Image.sent_at.desc()).first()
...
def get_image_info(image_id):
    ...
    image = Image.query.filter_by(id=image_id, recipient_id=current_user_id).first()
    if not image:
